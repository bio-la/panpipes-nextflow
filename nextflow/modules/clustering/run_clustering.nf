...
process clustering {
    ...
    output:
        ...
        path "logs/3-$modality-$sample-$algorithm-$resolution-run_clustering.log"
    ...
    python ${workflow.projectDir}/bin/run_clustering.py \
        --infile $input_h5mu \
        --modality $modality \
        --outfile clusters/$modality-$sample-$algorithm-$resolution-clusters.txt.gz \
        --resolution $resolution \
        --algorithm $algorithm \
        --neighbors_key $neighbors_key \
        > logs/3-$modality-$sample-$algorithm-$resolution-run_clustering.log
    """
}
