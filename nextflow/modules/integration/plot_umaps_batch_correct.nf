...
process plot_umaps_batch_correct {
    ...
    publishDir "${params.integration.outdir}/${params.integration.mode}/integration", mode: 'copy', overwrite: true, pattern: 'figures_umaps/**/*.png', saveAs: { file -> file }
    publishDir "${params.integration.outdir}/${params.integration.mode}/integration", mode: 'copy', overwrite: true, pattern: 'logs/plot_umaps_batch_correct.log'
    ...
    script:
    // Pass the plotting config (params.integration.plotqc) as inline JSON to --qc_dict
    // Using groovy.json.JsonOutput here ensures proper escaping
    def qc_json = groovy.json.JsonOutput.toJson(params.integration.plotqc)
